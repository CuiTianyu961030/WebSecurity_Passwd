...
#    
    ...
    password_num = 2000000
    with open(genPassfile,'w') as fw:
        for seed in tqdm(range(password_num)):
            structsign = seed
            genstruct = genString(seed,structADD_file,structsign)
            genPass = genPassword(seed,genstruct,outChar,outAlpha,outDigit)
            fw.write(genPass+'\n')
    fw.close()
    print('good job!')