...
#按照随机数的范围返回对应结构
def genString(seedpoint,structfile):
    stop = 1
    #求取上界
    maxValue = fileMax(structfile)
    #生成随机数
    randValue = genrandValue(seedpoint,maxValue)
    
    with open(structfile,'r') as fr:
        while( stop == 1 ):
            line = fr.readline().split(' ')
...
#根据生成的密码结构生成密码  
def genPassword(seedpoint,genstruct,outChar,outAlpha,outDigit):
...
#    print(genstruct)
    ...
    for item in genstruct:
        ...
        genPass += genString(seedpoint,choosefile)
        seedpoint += 1
    return genPass
...
#        writePercent(Tofile,inputPercentTuple)
    ...
    #结构字符串区间上界文件
    structADD_file = 'D:\\spyProject\\data\\web\\randomUSE\\passwd_ADDstruct.txt'    
#    normalizationStructFile(struct_file,structADD_file)
    
    #根据各字符长度生成txt文件
    original_alpha = 'D:\\spyProject\\data\\web\\passwd_alpha.txt'
    datadir_alpha = 'D:\\spyProject\\data\\web\\alpha\\'
    
    original_digit = 'D:\\spyProject\\data\\web\\passwd_digit.txt'
    datadir_digit = 'D:\\spyProject\\data\\web\\digit\\'
    
    original_char = 'D:\\spyProject\\data\\web\\passwd_char.txt'
    datadir_char = 'D:\\spyProject\\data\\web\\char\\'
...
#        filebyLen(originalfile,datadir)
    ...
    originalChar = 'D:\\spyProject\\data\\web\\char\\'
    medChar = 'D:\\spyProject\\data\\web\\medStrPercent\\char\\'
    outChar = 'D:\\spyProject\\data\\web\\randomUSE\\char\\'
    
    originalAlpha = 'D:\\spyProject\\data\\web\\alpha\\'
    medAlpha = 'D:\\spyProject\\data\\web\\medStrPercent\\alpha\\'
    outAlpha = 'D:\\spyProject\\data\\web\\randomUSE\\alpha\\'
    
    originalDigit = 'D:\\spyProject\\data\\web\\digit\\'
    medDigit = 'D:\\spyProject\\data\\web\\medStrPercent\\digit\\'
    outDigit = 'D:\\spyProject\\data\\web\\randomUSE\\digit\\'
    
#    for (originaldatadir,meddatadir,outdatadir) in [(originalChar,medChar,outChar),(originalAlpha,medAlpha,outAlpha),(originalDigit,medDigit,outDigit)]:
#        normalizationLengthFile(originaldatadir,meddatadir,outdatadir)
    
    genPassfile = 'D:\\spyProject\\data\\web\\mxm_genPasswordResult.txt'
    with open(genPassfile,'w') as fw:
        for seedpoint in range(0,1500):
            genstruct = genString(seedpoint,structADD_file)
            genPass = genPassword(seedpoint+2000,genstruct,outChar,outAlpha,outDigit)
            fw.write(genPass+'\n')
    fw.close()
    print('good job!')